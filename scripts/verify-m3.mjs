...
await page.waitForSelector(".preview-video, .notice-error, .status.error", { timeout: 90000 });

const failed = await page.evaluate(() => !document.querySelector(".preview-video"));
let b64 = null;
if (!failed) {
...
const panel = await page.evaluate(() => document.querySelector(".panel")?.innerText ?? "");
await browser.close();

if (!b64) {
  console.log("=== M3 FAILED to produce output ===\n" + panel);
...
const faceHadDetail = faceGradIn > 1.5;
const faceBlurred = faceGradOut < faceGradIn * 0.45;
const maskedToFace = faceMad > cornerMad * 3;
const ok = faceHadDetail && faceBlurred && maskedToFace && leaks.length === 0 && errors.length === 0;

console.log("=== M3 detection + blur verification ===");
console.log(
  JSON.stringify(
    {
      faceGradIn: faceGradIn.toFixed(2),
      faceGradOut: faceGradOut.toFixed(2),
      faceMad: faceMad.toFixed(1),
      cornerMad: cornerMad.toFixed(1),
      faceHadDetail,
      faceBlurred,
      maskedToFace,
      leaks,
